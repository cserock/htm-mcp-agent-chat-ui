...
//   const [groupName, setGroupName] = useState('');
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      if (isApproved) {
        ...
        toast.error('아직 승인되지 않은 계정입니다. 승인 후 사용하실 수 있습니다.');
        // 승인되지 않은 경우 로그아웃
        await signOut();
        router.push('/auth');
      }
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast.success('가입이 완료되었습니다. 입력하신 정보에 대해 승인 후 사용하실 수 있습니다.');
            ...
            <CardContent>
                {isFindEmailMode ? (
                    <div className="space-y-4">
                        <div className="text-center">
                            <h3 className="text-lg font-semibold text-gray-900">이메일 주소 찾기</h3>
                        </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <form onSubmit={handlePasswordReset} className="space-y-4">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="confirm-password">변경할 비밀번호 확인</Label>
                                <Input
                                    id="confirm-password"
                                    type="password"
                ...
                <TabsContent value="signin">
                    ...
                    <div className="text-center">
                        <button
                            type="button"
                            onClick={() => {
                                const email = prompt('가입하신 이메일 주소를 입력해 주세요.\n새 비밀번호를 생성할 수 있는 링크를 보내드립니다.');
                                if (email) {
                                    ...
                                    supabase.auth.resetPasswordForEmail(email, {
                                        ...
                                        if (error) {
                                            toast.error('이메일 전송에 실패했습니다: ' + error.message);
                                        } else {
                                            toast.success('비밀번호 설정 링크를 메일로 전송했습니다.');
                                        }
                                    });
                                }
                            }}
